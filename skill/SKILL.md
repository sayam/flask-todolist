...
description: Universal production-discipline rules for web applications, distilled from a real project's incidents — every rule carries the trap that created it. Includes stdlib-only checker scripts that enforce part of the rules mechanically. Rule text is in Thai.
---
...
**เกิดจาก:** semgrep เคยตรวจแค่ 71/136 ไฟล์เพราะขอบเขตประกาศสองที่ — ดัชนีที่ไม่ถูกบังคับให้ตรงกับความจริงคือดัชนีที่ให้ข้อมูลที่ไม่จริงเงียบ ๆ · audit รอบ 7 ต่ออีกฟิลด์ (`guards:` — ADR 0062): ด่านที่แพงและยังไม่เคยจับอะไรต้องบอกได้ ว่ามันคุ้ม*เส้นทางไหน* ไม่งั้นคำถาม "ยังคุ้มไหม" ตอบด้วยความรู้สึกเสมอ
...
**เกิดจาก:** ADR 0026/0030 — prod ที่ config ผิดแล้ว "ทำงานได้" บน SQLite จะล้มเหลวในวันที่ มีคนถามหาข้อมูลที่หายไป · ล้มเหลวดังตอน start ถูกกว่าเสมอ
...
**เกิดจาก:** มาตรฐานที่เปลี่ยนใต้เท้าทำให้ "ผ่าน" ของเมื่อวานไม่ใช่ของวันนี้โดยไม่มี commit ไหนบอก · หลักฐานที่ไม่ถูกตรวจว่ามีจริงจะล้าสมัยเป็นรายการแรก
...
**เกิดจาก:** ดัชนีเคยหยุดที่ 0026 ขณะที่ไฟล์มีถึง 0033 — เจ็ดใบจากเฟสที่ตัดสินใจ เรื่องใหญ่ที่สุดหายจากสายตา · audit รอบ 14 เพิ่มทิศที่สอง: ADR 0035 แทน ข้อ 1 ของ 0032 ตั้งแต่ 2026-08-12 แต่หน้าของ 0032 ไม่รู้ตัวเจ็ดวัน ขณะที่ docstring หัวไฟล์ของ app/audit.py ชี้มาที่ 0032 ในฐานะกลไกปัจจุบัน ทั้งที่ CLAUDE.md ห้ามกลับไปใช้กลไกนั้นตรง ๆ
...
**กฎ:** LICENSE เป็นของจริงตามที่ ADR ประกาศ · core ไม่มี dependency ที่แบกภาระ copyleft

**เกิดจาก:** ldap3 เป็น LGPLv3 และอยู่ใน category ของ plugin พอดี — การแยก supply chain แยกภาระทางกฎหมายไปด้วยโดยไม่มีใครออกแบบ · "ไม่พบ" ≠ "อ่านไม่ได้จึงไม่พบ" · ADR 0070 (2026-08-19) เปลี่ยนสัญญาอนุญาตเป็น AGPL-3.0 + CC BY-SA 4.0 — ด่านนี้จึงตรวจเนื้อของ AGPL (รวมข้อ 13 เรื่องการให้บริการผ่านเครือข่าย ซึ่งเป็นเหตุผลทั้งหมดที่เลือกใบนั้น) และตรวจว่า LICENSE-docs มีจริง · **ส่วนที่ตรวจ copyleft ของ dependency ไม่เปลี่ยน** เพราะภาระนั้นเป็นของ ผู้ใช้ปลายทาง ไม่ใช่ของเรา
...
**เกิดจาก:** เอกสารสามที่บอก "CI 21 job" ขณะที่ workflow นิยาม 20 — ไม่มีใครให้ข้อมูลที่ไม่จริง แต่ไม่มีใครตรวจ · 2026-08-17 ซ้ำอีกชั้น: ใบตอบ badge ยังเขียน v1.5.0 สามจุดหลังออก v1.6.0 แล้ว ทั้งที่ runbook มีขั้นตอนให้อัปเดต — **ขั้นตอน ที่ไม่มีเทสต์อ่านคู่ ก็ล้าสมัยเหมือนตัวเลขที่ไม่มีเทสต์อ่านคู่**
...
**กฎ:** สภาพหลัง clone (ไม่มีไลบรารี plugin) ต้องเขียว — "ถอดแล้วระบบยังทำงานได้" วัดได้
...
**เกิดจาก:** rolling deploy มีจังหวะที่สองรุ่นรันพร้อมกันบน schema เดียวเสมอ — migration ที่ถูกต้องสำหรับโค้ดใหม่แต่ฆ่าโค้ดเก่าคือ downtime ที่ไม่มีใคร ประกาศ · สัญญาที่ไม่มีด่านรันหนุนคือสัญญาที่รอวันล้าสมัย (ADR 0048)
...
**กฎ:** pa11y + Chromium จริง สแกนรวมโหมดมืด ธีมเสริม และภาษาไทย
...
**เกิดจาก:** Dockerfile ที่ไม่มีใครเคย build คือไฟล์ข้อความธรรมดา — และ shebang ของ venv เคยล้มเหลวแบบที่ข้อความ error ให้ข้อมูลที่ไม่จริงว่าไฟล์ไม่มีอยู่
...
**กฎ:** พื้นของ ratchet ติดกับของจริงสองทิศ · และของที่ถูกถอดออกต้องมีคนเซ็นชื่อ

**เกิดจาก:** audit รอบ 12 — `pyproject.toml` เขียนกำกับว่า "ขยับขึ้นได้อย่างเดียว" ทั้งสองที่ แต่ไม่มีอะไรทำให้มันขยับ · หกวันหลังตั้งเลข coverage จริงไต่ไป 97.11% ขณะที่พื้น ยังเป็น 96 (ตั้งตอนวัดได้ 96.31%) — ที่ว่าง 1.11 จุดคือโค้ดที่มีเทสต์คุมราว 54 บรรทัดซึ่งหายไปได้โดยไม่มีอะไรแดง · กลไกที่ถูกมีอยู่แล้วในโปรเจกต์นี้ (`LINE_SLACK` ของ ADR 0065) แค่ยังไม่เคยถูกเอาไปใช้กับ ratchet ตัวอื่น · audit รอบ 14 เพิ่ม strict list ของ mypy เข้ามา — ratchet ที่เขียนเป็น ประโยครอดจากตัวทวงรุ่นแรกได้ เพราะตัวทวงอ่านเฉพาะพื้นที่เป็นตัวเลขใน config ของเครื่องมือ · ของจริงครอบ 34/72 โมดูล ขณะที่เป้าที่เขียนกำกับ หมดอายุไปสิบหกเฟส · audit รอบ 16 เพิ่มกองที่กันการ *ถอด* (ADR 0069): วัดด้วยการลบของจริง 11 ครั้งแล้วพบว่าถอด gate ทิ้งได้โดย CI เขียวครบถ้าเก็บกวาด 6 ที่ และ 37 แถวในสามทะเบียนกระดาษลบได้เงียบสนิท เพราะการลบทั้งสองข้างพร้อมกันยังนับว่า "ตรงกัน" อยู่ดี

**ตัวบังคับใน reference:** job `test` step "พื้นของ ratchet ต้องไม่ลอยต่ำกว่าของจริง"
