...
3. **พิสูจน์สองทิศก่อนเข้า CI** (บนเครื่อง — hadolint เป็น static binary
   ...
   `MAINTAINER` = exit 1
4. **บทเรียนจากรอบแรกของด่านนี้เอง**: เครื่องลองด้วย 2.14.0 แต่ action
   bundle 2.15.x ซึ่งเจอเพิ่มสองข้อที่รุ่นเก่าไม่จับ (DL3066 `USER` ควร
   เป็น uid ตัวเลข · DL3025 HEALTHCHECK CMD ควรเป็น JSON form) — ทั้งคู่
   **แก้จริงไม่ยกเว้น** · รุ่นที่ตัดสินคือรุ่นใน CI เสมอ การลองในเครื่อง
   เป็นแค่ preview (สัญญาณผิดตัวแบบเดียวกับบทเรียนเฟส 5)
