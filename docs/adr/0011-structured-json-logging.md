...
- `request_id` รับต่อจาก header `X-Request-Id` ได้ **แต่ต้อง parse เป็น UUID ได้
  เท่านั้น** ค่าที่ไม่ถูกต้องจากภายนอกถูกทิ้งแล้วสร้างใหม่ — ไม่งั้นเป็นช่องปลอมแปลง/
...
สิ่งที่เกิดขึ้นจริงตอน P5-07: เพิ่มคำเตือนที่ดังทุกครั้งที่ start แล้ว CI ล้มเหลวห้า job
