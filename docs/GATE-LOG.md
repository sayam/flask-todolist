...
คำสั่งและใน `env:` · job ที่ต้องมี docker (`image` · `stack` · `dast` ฯลฯ) ไม่อยู่
ในขอบเขตเลยและประกาศไว้ตรง ๆ ว่า "preflight เขียว ≠ CI เขียว"
...
**ข้อยกเว้นเดียวของกฎ "env เดินทางไปกับคำสั่ง" คือ service container**
(2026-08-27) — job `test` ประกาศ `services.redis` ไว้ CI จึงยื่นที่อยู่ของ
container ที่มันสตาร์ตให้ (`TEST_REDIS_URL=redis://127.0.0.1:6379/0`) ·
บนเครื่องที่ไม่มี redis ที่อยู่นั้นไม่ได้แปลว่าอะไรเลย ชุดเทสต์จึงไปเคาะพอร์ต
ที่ไม่มีใครรับแล้วรายงานความล้มเหลวที่ไม่เกี่ยวกับสิ่งที่กำลังแก้ — **วัดจริง:
preflight แดงที่ job `test` บนกิ่งที่ CI เขียวครบ 28 check**

ตอนนี้ `plan()` probe พอร์ตที่ `services:` ประกาศไว้ก่อน · **มีคนรับ = ใช้ env
เหมือน CI ทุกอย่าง** (สตาร์ต redis เองแล้วได้ความเที่ยงเต็ม) · **ไม่มีคนรับ =
ถอนเฉพาะตัวแปรที่ถือพอร์ตนั้น แล้วรันสเต็ปต่อ ไม่ข้ามทั้งสเต็ป** เพราะการข้าม
คือทิ้งชุดเทสต์ 1,981 ตัวเพื่อรักษาสองตัว ซึ่งเป็นการแลกที่แย่กว่าปัญหาเดิม ·
เทสต์ที่ต้องใช้ redis ข้ามตัวเองตาม `skipif(not TEST_REDIS_URL)` ที่ประกาศไว้
อยู่แล้ว — สภาพนี้จึงเป็นสภาพที่โปรเจกต์ออกแบบไว้เอง ไม่ใช่สภาพที่ preflight
คิดขึ้นใหม่ · **การรันแบบนี้ถูกรายงานแยกจาก pass สะอาด** (`~~` + ตัวนับ
`ran without a service CI provides` ในบรรทัดสรุป) เพราะการรันที่ขาดของที่ CI มี
ต้องไม่อ่านเหมือนการรันที่มีครบ (vg PR #29)
