...
> **สถานะ: เอกสารนี้ถูกใช้ไปแล้วและงานทำครบแล้ว (2026-08-14)** — แผนที่มันสั่งให้
> ผลิตอยู่ที่ [`ROADMAP-INFRA.md`](ROADMAP-INFRA.md) และเฟส 8–12 ปิดครบทั้งหมด
> · เก็บไฟล์นี้ไว้เป็น **input ต้นทาง** เพื่อให้ตรวจย้อนได้ว่าอะไรถูกรับ อะไรถูก
> ปฏิเสธ และเพราะอะไร (ตารางคำตัดสินรายข้ออยู่ในหัวข้อ 1 ของ `ROADMAP-INFRA.md`)
> **ไม่ใช่เอกสารที่บอกสถานะปัจจุบัน** — ข้อเท็จจริงหลายข้อในนี้ถูกแก้โดยการสำรวจ
> จริงตอนวางแผน (เช่น migration class เป็นของ *port* ไม่ใช่ของ *type*)
>
...
3. เขียน ADR ร่างสำหรับข้อ 4 (สิ่งที่ตัดออก) และสำหรับการตัดสินใจโครง
   `gates.yaml`
4. กติกาเดิมของ repo คงเดิมทั้งหมด: coverage 97% ขึ้นทางเดียว, เทสต์ใหม่ต้อง
