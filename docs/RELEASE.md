...
### ถ้าต้องหมุน token ของ Zenodo

token อยู่ใน **query string ของ URL ปลายทางของ webhook** (ดีไซน์ของ Zenodo เอง)
`gh api repos/:owner/:repo/hooks` จึงคืนมันมาเป็น plaintext — เห็นได้เฉพาะคนที่
เป็น admin ของ repo แต่ก็แปลว่ามันหลุดออกไปนอกช่องทางที่ตั้งใจได้ง่ายกว่า
ความลับตัวอื่น (คำสั่งเดียวที่ใครสั่งให้รันก็ได้ · log ของ terminal · transcript)

หมุนแล้วต้อง**ต่อสายใหม่** ไม่ใช่แค่เพิกถอน — ไม่งั้น release ถัดไปเงียบหายไป
โดยที่ทุกอย่างดูปกติ:

1. Zenodo → Applications → Personal access tokens → เพิกถอนใบเดิม
2. Zenodo → GitHub → ปิดสวิตช์ของ repo แล้วเปิดใหม่ (ขั้นนี้คือขั้นที่ติดตั้ง
   webhook ใบใหม่พร้อม token ใหม่)
3. ยืนยันว่าสายใหม่ต่อแล้วจริง — **`id` ต้องเป็นเลขใหม่** ไม่ใช่แค่ `length` เท่าเดิม:
   ```
   gh api repos/:owner/:repo/hooks --jq '.[] | {id, active, events}'
   ```
4. release ถัดไปตรวจที่ Zenodo ตามคำสั่งข้างบน — การหมุนที่ทำพลาดจะรู้ตัวตอนนั้น
