...
|---|---|---|
| เจ้าของ/ผู้พัฒนา | แก้แล้วไม่กระทบของเดิม · บทเรียนไม่สูญ | Development · ทุก ADR |
...
### ธรรมนูญ: ลำดับความสำคัญของความกังวล (`ADR 0051`)

ความกังวลข้างบนถูกจัดลำดับเป็นสี่ชั้น — ชนกันเมื่อไหร่ ชั้นบนชนะโดยไม่ต้อง
เถียงใหม่ · ยกเว้นชั่วคราวต้องมี ADR พร้อมเงื่อนไขหมดอายุ:

1. **Security** — กฎหมายรายประเทศ (ดัชนี `docs/COMPLIANCE.md` · ไทย/PDPA —
   `docs/PDPA.md`) ·
...
ทุก gate ใน `gates.yaml` ประกาศ pillar ว่ารับใช้ชั้นไหน
(`tests/test_gates.py` บังคับครบ) · ของใหม่จากภายนอกเข้าผ่านกระบวนการ
intake ใน `CONTRIBUTING.md` (กฎข้อ 10) — baseline ห้าม break เป็นข้อแรกเสมอ
...
ที่ไฟล์นี้อ้าง — การอ้างอิงที่ใช้ไม่ได้ไม่ต้องรอรอบ: เทสต์จับทันทีที่ไฟล์/ADR หาย
