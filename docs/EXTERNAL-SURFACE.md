...
| repo `sayam/verifiable-gates` (ADR 0075 ข้อ 6) | public · Apache-2.0 + CLA · CC BY 4.0 · **branch protection ตั้งแล้ว 2026-08-25 รูปเดียวกับที่นี่** (required check `lint`/`test`/`commit-lint` · `enforce_admins` · linear · ห้าม force push/ลบ · ต้อง resolve conversation · review 0) พิสูจน์ด้วยการลอง push ตรงแล้วถูกปฏิเสธ `GH006` · **แต่ยังไม่มีเครื่องตรวจว่ามันยังเป็นแบบนั้นอยู่** — *กลไก* ของ `audit_posture` อยู่ที่ vg (`verifiable_gates.posture`) ตั้งแต่ขั้น 3e · ที่นี่เหลือ adapter ซึ่งชี้ที่ repo นี้ ไม่ได้ตรวจ posture ของ vg · **ช่อง About · topics · homepage ของ repo นั้น กรอกแล้ว 2026-08-26 · description ขยับเป็น v0.1.0 เมื่อ 2026-08-28 (รุ่นแรก · DOI `10.5281/zenodo.22145684`)** (description ตรงกับสิ่งที่มีจริง · topics ห้าตัวสะท้อน `keywords` ใน `CITATION.cff` บวก `python` ที่เป็นภาษา · homepage ชี้ DOI) — **และไม่มีอะไรอ่านคู่ เหมือนกัน** ซึ่งเป็นชั้นเดียวกับใบตอบ badge ข้างล่าง | ยังไม่มีใคร |
