...
(4) **บอกว่าเขา*หยุดทำให้เรา*เมื่อไหร่** โดยที่ตัวเขาเองไม่ได้เปลี่ยนอะไรเลย

**ข้อ (4) เกิดจาก audit รอบ 27** — สามข้อแรกถามถึงความล้มเหลวที่ *ดัง*: เขาเปลี่ยน
สัญญา ของพัง job แดง · แต่ความล้มเหลวที่กัดจริงในรอบ 26 เป็นอีกชนิดหนึ่ง —
**เขายังทำงานปกติทุกอย่าง แต่เลิกทำให้เรา** ตามนโยบายที่เป็นเรื่องของ *เรา* ไม่ใช่
ของเขา: Dependabot หยุดเปิด PR เมื่อไม่มีใครแตะของมันครบ 90 วัน · GitHub ปิด
`on: schedule` ของ repo สาธารณะที่เงียบครบ 60 วัน · alert ที่ dismiss แล้วจะไม่ถูก
เปิดคืนแม้เนื้อของมันจะกลับมาเป็นจริง · **ไม่มีอะไรแดงในทั้งสามกรณี เพราะไม่มี
อะไรเกิดขึ้นเลย** — และตอนตรวจรอบ 27 พบว่า 10 จาก 10 แถวตอบข้อ (2) ครบ ส่วนข้อ (4)
ตอบได้ **0 แถว** · ที่ชี้ชัดที่สุดคือ Dependabot ซึ่งเป็นผู้ให้บริการที่พิสูจน์แล้วว่า
หยุดให้เราได้ กลับไม่มีแถวของตัวเอง — มันโผล่อยู่ในช่อง "อะไรจะแดง" ของ Debian

| ผู้ให้บริการ | เราพึ่งอะไร | อะไรจะแดงถ้าเขาเปลี่ยน | เขาหยุดทำให้เราเมื่อไหร่ (โดยตัวเขาไม่เปลี่ยน) |
|---|---|---|---|
| GitHub — branch protection, auto-merge, Actions permissions | กติกาที่ ADR 0053 ประกาศ: PR-only, enforce_admins, required check ครบ | job `posture` (ADR 0061) เทียบ API กับที่ประกาศทุกครั้งที่กฎเปลี่ยน ทุก push บน main และกดรันเองได้ (`workflow_dispatch`) · อ่านด้วย secret `POSTURE_TOKEN` (PAT อ่านอย่างเดียว) · ฟิลด์ที่ token อ่านไม่ได้รายงานว่า **มองไม่เห็น ไม่ใช่ปิดอยู่** และมีแถว cadence ตรวจด้วยมือแทน | **60 วัน** — GitHub ปิด `on: schedule` ของ repo สาธารณะที่ไม่มีความเคลื่อนไหว แล้ว job `posture` ที่เฝ้าท่าทีนี้เลิกรัน · กันด้วยแถวรักษาชีพรอบ 45 วัน ([ADR 0074](adr/0074-watcher-windows-fit-platform-silence.md)) |
| GitHub Actions — runner และ codeload (actions/checkout, actions/setup-python, actions/setup-node, actions/upload-artifact) | ทุก job ใน CI เริ่มด้วยการโหลด action จากที่นี่ | job นั้นแดงทันที — **ไม่จำเป็นต้องแดงที่ `Set up job`** (เกิดจริง 4 ครั้งวันที่ 2026-08-17/18: `codeql` แดงที่ step ของ action เอง โดยข้างในเป็น HTTP 503) · แยกออกจาก flake ของด่านเราด้วย `scripts/rerun_census.py` ซึ่งอ่าน**ข้อความ**ของความล้มเหลว ไม่ใช่ชื่อ step และส่งของที่จำแนกไม่ได้ไปชั้น `ต้องอ่านเอง` · ขั้นตอนตัดสินก่อนกด rerun อยู่ใน `docs/OPERATIONS.md` | ไม่มีที่รู้จัก — repo สาธารณะไม่มีโควตานาที และ codeload ไม่มีนโยบายหยุดให้ repo ที่เงียบ |
| GitHub — attestation API และ actions/attest-build-provenance | provenance ของ release ที่ผู้ใช้ verify ได้ | job `release-sign` แดงตอนออกรุ่น (verify สองทิศก่อนแนบ) | ไม่มีที่รู้จัก — ใช้เฉพาะตอนออกรุ่น ซึ่งเป็นจังหวะที่มีคนลงมืออยู่แล้ว |
| sigstore — Fulcio/Rekor และ sigstore/cosign-installer | ลายเซ็น keyless ของ SBOM ทุกไฟล์ | job `release-sign` แดงตอนออกรุ่น | ไม่มีที่รู้จัก — เหมือนแถวบน ใช้เฉพาะตอนออกรุ่น |
| เครื่องมือสแกนที่เป็น action — aquasecurity/trivy-action, github/codeql-action, gitleaks/gitleaks-action, hadolint/hadolint-action, ossf/scorecard-action, grafana/setup-k6-action | ด่าน image, SAST, secret scan, lint ของ Dockerfile, คะแนน posture, tripwire ของ performance | job `image`, `codeql`, `secret-scan`, `lint`, `scorecard`, `perf-smoke` แดง — และรุ่นที่ตัดสินคือรุ่นใน action ไม่ใช่รุ่นบนเครื่อง (ADR 0055) | ไม่มีที่รู้จักต่อ repo เรา — แต่ action ที่ถูก archive จะค้างรุ่นเงียบ ๆ ซึ่ง Dependabot เป็นคนขยับ ดูแถวของมัน |
| Debian ผ่าน base image python:3.13-slim | ชั้น OS ของ image ที่ deploy จริง | `gate image-os-cve-audit` (trivy) + Dependabot ecosystem `docker` | **เมื่อ Debian รุ่นนั้น EOL** — `gate image-os-cve-audit` นับเฉพาะ CVE **ที่มี fix** ด่านจึงเงียบลงเรื่อย ๆ ขณะที่ความเสี่ยงเพิ่มขึ้น · `docs/eol-pinned.json` ยังตรึงแต่ของ Python ไม่มีของ Debian — มีแถวทวงรอบ 6 เดือนใน `docs/SECURITY-CADENCE.md` (วันหมดอายุของ runtime) |
| **Dependabot** — ตัวขยับของทุกอย่างที่เราตรึงไว้ | `pins/` ทุกไดเรกทอรี · digest ของ base image และ image ของ stack · รุ่นของ action · และ **security update ของ pip ที่ `.github/dependabot.yml` อ้างเป็นตาข่าย** | `gate dependabot-fits-the-gates` ตรวจว่า config ยังตรงกับสิ่งที่เราตรึง — แต่มันตรวจ *ไฟล์* ไม่ใช่ *ว่ามันยังทำงานให้เราอยู่ไหม* | **90 วัน** — GitHub ถือว่า repo ไม่ active เมื่อไม่มีใครแตะ PR ของมันครบ 90 วัน แล้ว**หยุดเปิด PR ทั้ง version และ security update** · ตาข่ายกับของที่มันรับหายไปพร้อมกันด้วยเงื่อนไขเดียว (audit รอบ 26 ข้อ 3) · กันด้วยแถวรักษาชีพรอบ 45 วัน |
| PyPI และ npm | ไลบรารีทุกชั้น: core, deploy, plugin, เครื่องมือของ CI | `gate core-deps-cve-audit`, `gate ci-tools-cve-audit`, `gate plugin-deps-cve-decided` | ไม่มีที่รู้จัก — การ yank รุ่นทำให้ `--require-hashes` แดงดัง ไม่ใช่เงียบ |
| Docker Hub และ quay.io — image ของ stack ที่ CI ยิงจริง (mysql, mariadb, redis, nginx, prom/prometheus, grafana/grafana, grafana/loki, grafana/alloy, hashicorp/vault, quay.io/keycloak/keycloak, bitnamilegacy/openldap) | stack จริงที่ job หลายตัวยิงใส่ทุก push (11 จาก 25 job) | **ความพร้อมใช้**: job แดงทันทีที่ดึง image ไม่ได้ · **ความสมบูรณ์และความทำซ้ำได้**: ตรึงด้วย digest ทุกตัวตั้งแต่ audit รอบ 15 (`gate stack-images-pinned-and-moved`) — ก่อนหน้านั้นเป็น tag ล้วน และ image ของ ZAP ซึ่งเป็น *ตัวตัดสิน* ผลด้านความปลอดภัย ใช้ tag ลอย (`stable`) ผลเขียวจึงทำซ้ำไม่ได้ | ไม่มีที่รู้จักแบบเงียบ — โควตา pull แบบไม่ล็อกอินชนแล้ว job แดงทันที |
| bestpractices.dev (OpenSSF Best Practices) | badge สามระดับที่ `README.md` โฆษณา และใบตอบใน `docs/BEST-PRACTICES.md` | `ci:posture` เทียบเปอร์เซ็นต์ทั้งหกชุดกับที่เอกสารประกาศทุก push บน main (audit รอบ 27) · ส่วนคำตอบราย*ข้อ* ทั้ง 122 ช่องยังเป็นแถวทบทวนประจำปีใน `docs/SECURITY-CADENCE.md` | **เมื่อเขาแก้เกณฑ์** — คำตอบที่เคยผ่านกลายเป็นไม่ผ่านโดยที่เราไม่ได้ทำอะไร และ badge ลดระดับเงียบ ๆ · **ปิดแล้วใน audit รอบ 27 ข้อ 3**: `scripts/audit_posture.py` อ่านใบตอบ JSON ของโครงการ #14085 สดทุกครั้งที่ `ci:posture` รัน แล้วเทียบกับตารางเปอร์เซ็นต์ใน [BEST-PRACTICES.md](BEST-PRACTICES.md) — badge ที่ถูกลดระดับจึงเป็นความแดง ไม่ใช่ความเงียบ |
| Zenodo — archive ของทุก release, DOI ที่ `README.md`/`CITATION.cff` โฆษณา และ webhook ที่ active อยู่บนรีโปนี้ | ความคงอยู่ของ DOI · เนื้อของระเบียนที่ตีพิมพ์ (อ่านจาก `.zenodo.json` + `CITATION.cff` ตอน archive) · token ของ webhook ที่ฝังอยู่ใน query string ของ URL (แถวความเสี่ยงใน [RISK-ASSESSMENT.md](RISK-ASSESSMENT.md)) | `tests/test_identity_cards.py` บังคับว่าสองใบที่เขาอ่านตรงกันเองและตรงกับ [AUDIT-LOG.md](AUDIT-LOG.md) — **แต่ระเบียนที่ตีพิมพ์ไปแล้วไม่มีอะไรอ่านคู่** และแก้ย้อนหลังไม่ได้ตามนิยามของการ archive · ขั้นตอนตรวจด้วยมืออยู่ใน [RELEASE.md](RELEASE.md) · **แถวนี้เพิ่มตอน audit รอบ 24**: ทะเบียนความเสี่ยงรู้จัก Zenodo มาก่อนแล้ว ส่วนทะเบียนนี้ไม่รู้จัก — ทะเบียนสองใบที่ไม่ตรงกันว่าเราพึ่งใครอยู่ ทำให้ด่านสองทิศของแกนนี้บังคับความครบของรายการที่ตัวมันเองไม่รู้ว่ายังขาดใคร | **เมื่อ webhook ถูกปิด/token ใช้ไม่ได้** — release ถัดไปไม่ถูก archive และไม่มีอะไรแดง เพราะความล้มเหลวคือ *ไม่มีระเบียนใหม่โผล่มา* · ขั้นตอนถาม Zenodo หลังออกรุ่นอยู่ใน `docs/RELEASE.md` |
