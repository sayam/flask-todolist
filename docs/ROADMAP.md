...
| OpenSSF Scorecard | ✅ 2026-08-13 | `.github/workflows/scorecard.yml` · ผลไปโผล่ที่หน้า Security ร่วมกับ CodeQL และเผยแพร่ไป OpenSSF API เพื่อให้มี badge · **ไม่อยู่ในรายการ required check** เพราะไม่รันบน PR และเป็นคะแนนไม่ใช่ผ่าน/ไม่ผ่าน · action ทุกตัวใน workflow ถูก pin ด้วย commit SHA แล้ว (`tests/test_workflow_pinning.py` บังคับ) ซึ่งปิดข้อ `Pinned-Dependencies` ของ Scorecard ด้วย |
