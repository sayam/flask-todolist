...
- **แถม:** แก้ต้นเหตุที่ `TestConfig` ไม่สืบทอด `Config` (เกิดปัญหาซ้ำมาแล้ว 4 ครั้ง)
...
ใน `url_map` (mutation test 18 แบบ — จับได้ทั้งหมด)
...
- **Session ✅ (ADR 0020):** ล้าง session ทั้งใบตอน login (session fixation),
  ...
  คุกกี้ทุกใบที่ออกก่อนหน้า **รวมใบที่อยู่ในมือคนอื่น** ใช้ไม่ได้พร้อมกัน
...
(ยืนยันซ้ำแล้วถอย `last_counter` กลับ = เปิดช่องใช้รหัสซ้ำ) แล้วมันหยุดทำงาน
...
- **job `bare` ใน CI ✅** — รันชุดเทสต์โดย**ไม่ติดตั้ง category ของ plugin เลย**
  ต้องเขียว นี่คือด่านเดียวที่ทำให้ "ถอดแล้วระบบยังทำงานได้" เป็นข้อเท็จจริงที่วัดได้ทุก push
...
**สิ่งที่เฟสนี้สอนซ้ำจนเป็นรูปแบบ**: ด่านที่ "มีอยู่" กับด่านที่ "ครอบชั้นที่เกิดปัญหาจริง"
...
**ต้องทดสอบสองทิศ** (ล้มเหลวเมื่อควรล้มเหลว และผ่านเมื่อควรผ่าน)
...
ด่านที่ "ครอบชั้นที่เกิดปัญหาจริง" · เฟส 6 เติมอีกข้อว่า **ตัวเลขจากการวัดรอบเดียวไม่ใช่
...
Phase 5 สอนว่าด่านที่ "มีอยู่" ไม่เท่ากับด่านที่ครอบชั้นที่เกิดปัญหาจริง และความต่าง
...
ว่าแก้ถูก ก็ให้ข้อมูลที่ไม่จริงได้** — repro ที่ใช้ยืนยันว่า deadlock หายไปเรียก `audit.record()`
...
ให้เหมือนคำขอจริง ตัวเลขเปลี่ยนจาก "แก้แล้ว" เป็น "ยังล้มเหลวอยู่ 87%"**
...
สำคัญ (แถวล็อกของสาย audit หายไปต้องดัง · plugin ตัวเดียวล้มเหลวต้องไม่ล้มทั้งคำขอ)
...
## สรุปว่าอะไรเปลี่ยนไปหลังเฟส 5–7 (ย้ายมาจาก `CLAUDE.md` เมื่อ 2026-08-20)
...
แล้วว่าดังจริง** · และเอกสารทุกฉบับที่เพิ่มมามีเทสต์คุมไม่ให้ล้าสมัย
...
|---|---|---|
| การออกแบบฐานข้อมูลเทียบแนวปฏิบัติที่ดี (ดัชนีผสม · pagination · collation · พาร์ทิชันของ audit · ตัวชี้วัดชั้นฐานข้อมูล · การซ้อมกู้คืนของ MySQL/MariaDB ฯลฯ) | **ทบทวนแล้ว 2026-08-20 · บันทึกไว้ 10 ข้อ ยังไม่ลงมือ** — รายละเอียดและเงื่อนไขที่ทำให้แต่ละข้อถึงคิวอยู่ใน [`DATABASE-REVIEW.md`](DATABASE-REVIEW.md) | ตามเงื่อนไขรายข้อ (ส่วนใหญ่ผูกกับการวัด ไม่ใช่กับปฏิทิน) |
