...
ต่อรอบที่ 25 VUs หลังแก้ ล้วนเป็นสิ่งที่ลองใหม่ได้ตามนิยาม) · ปรับ `--workers`
ของ gunicorn แล้ววัดใหม่ (ตัวเลขทั้งรายงานเป็นของ worker เดียวต่อ container
ซึ่งเป็นสภาพที่คน deploy ตามเอกสารจะได้จริง) · ยังไม่มี Prometheus/Grafana
ที่ scrape จริงเป็นรอบ — `/metrics` มีของให้ scrape แล้วแต่ยังไม่มีใครดูดไปเก็บ
...
**ผลการประเมิน ASVS: ผ่าน 138 · ไม่เกี่ยวข้อง 67 · ยังไม่ผ่าน 48**
...
- **ยังไม่วัด throughput ใหม่หลังเปลี่ยนวิธีล็อกและระดับการแยก** — ตัวเลขใน
  PERFORMANCE.md วัดกับล็อกที่ไม่ทำงาน จึงเทียบกับของตอนนี้ตรง ๆ ไม่ได้
...
| Branch protection | ✅ 2026-08-13 | บังคับ PR + **ครบทั้ง 23 check** + linear history · ห้าม force push และห้ามลบ branch · **`enforce_admins: false`** ยกเว้นเจ้าของจาก **ทุกข้อ** ไม่ใช่แค่ข้อบังคับ PR (โปรเจกต์คนเดียว การบังคับ PR กับตัวเองคือความช้าที่ไม่ซื้ออะไร — แต่ต้องรู้ว่ามันยกเว้น force push ให้ด้วย) · ต้อง approve 0 คน เพราะคนเดียว approve PR ตัวเองไม่ได้ — ตั้ง 1 เมื่อไหร่คือ merge ไม่ได้ตลอดกาล |
...
| Dependabot security updates | ✅ 2026-08-12 | เปิด PR อัตโนมัติเมื่อมี CVE · **มองไม่เห็น category ของ plugin และ `[deploy]`** เพราะ dependency graph อ่านแค่ `[packages]`/`[dev-packages]` — วัดแล้วเห็น 126 package · ช่องนั้นปิดด้วย job `plugin-audit` และ `security` (ดู [SECURITY-CADENCE.md](SECURITY-CADENCE.md)) · **version updates ยังไม่เปิด** |
| OpenSSF Scorecard | ✅ 2026-08-13 | `.github/workflows/scorecard.yml` · ผลไปโผล่ที่หน้า Security ร่วมกับ CodeQL และเผยแพร่ไป OpenSSF API เพื่อให้มี badge · **ไม่อยู่ในรายการ required check** เพราะไม่รันบน PR และเป็นคะแนนไม่ใช่ผ่าน/ไม่ผ่าน · `ossf/scorecard-action` **ไม่มี tag `@v2`** ต้อง pin เต็มรุ่น |
