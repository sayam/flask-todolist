...
> **ก่อนออกรุ่นถัดไป ยังมีช่องบนเว็บที่ต้องแก้** — ดูหัวข้อ
> "ค่าที่เก็บอยู่บนเว็บจริง" (ตรวจกับ API แล้ว ไม่ใช่จากความจำ)
...
| `description` | "… v2.2.0 … **112** machine-checked gates, 72 ADRs, **27** recorded governance audits …" | **ค้างอีกแล้ว** — ของจริงวันนี้คือ 114 gate · 78 ADR · 27 รอบ · ช่อง About ของ repo ถูกซิงก์แล้ว (2026-08-26 ด้วย `scripts/sync_counts.py --about --write`) เหลือใบตอบบนเว็บ badge ที่ยังต้องไล่ด้วยมือ · เคย **ตรงกันสามที่ (2026-08-23)** — เว็บ badge · ช่อง About ของ repo · และของบนดิสก์ (`gates.yaml` · `docs/adr/` · [AUDIT-LOG.md](AUDIT-LOG.md)) — แล้วห่างอีกครั้งภายในสามวัน ซึ่งเป็นคำตอบของ audit รอบ 24 ที่ชัดที่สุด: ฝั่ง About มี `ci:posture` อ่านคู่ให้ตั้งแต่ ADR 0072 จึง*รู้ตัว* ส่วนฝั่งเว็บ badge ไม่มีใครอ่านคู่เลย |
