...
| CI/CD | 23 job ยิงของจริงทุก push — สามยี่ห้อฐานข้อมูล, stack จริง, IdP จริง | `.github/workflows/ci.yml` · required checks |
...
| Observability | log JSON + request id · `/metrics` ต้องมี token · SIEM stack ที่ alert ดังจริง | `app/logging_setup.py` · `app/metrics.py` · `ci:siem` |
...
| review โดยคนที่สอง | required checks 24 ตัวแทน (ผู้ดูแลคนเดียว — GitHub ไม่ให้ approve PR ตัวเอง) | ตั้ง approvals ≥1 กับคนเดียว = merge ไม่ได้ตลอดกาล · เงื่อนไขทบทวน: มีผู้ดูแลคนที่สอง |
