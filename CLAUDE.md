...
- **นับความล้มเหลวของ CI ด้วย `scripts/rerun_census.py` เท่านั้น** (audit r7) —
  ...
  ตัวนับแยก **ความล้มเหลวของแพลตฟอร์ม** (ล้มที่ step `Set up job` เช่น GitHub
  ตอบ 429 ตอนโหลด action) ออกจากของเรา เพราะเกณฑ์ flake ของด่านเราต้องไม่ถูก
  มลพิษด้วยเรื่องที่เราแก้ไม่ได้
