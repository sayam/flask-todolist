...
> **เพดานของไฟล์นี้: 1,200 บรรทัด / 8,100 คำ** (ADR 0065) — ไฟล์นี้ถูกอ่านทั้งไฟล์
...
- `app/health.py` — `/healthz` (liveness — **ไม่แตะ DB โดยตั้งใจ**) + `/readyz`
  ...
  healthcheck ทับใน compose** (เคยทำแล้ว stack TLS ใช้การไม่ได้ทั้งแถบเพราะ header หาย)
...
- `app/metrics.py` — latency histogram ต่อ endpoint + `/metrics` (Prometheus)
  **ต้องมี token เสมอ** (ด่านเดียวกับ `/api/v1`) · **label เป็นชื่อ endpoint
  ไม่ใช่ `request.path`** ไม่งั้นคนนอกยิง path ที่ไม่มีอยู่จริง ให้ time series ระเบิดได้
...
- `compose.yaml` + `compose.{mysql,mariadb,scale,sso}.yaml` — stack ที่รันจริง
  ...
  **`compose.tls.yaml` ต้องต่อจาก `compose.scale.yaml`** (TLS เป็นของ proxy)
  และถือทั้งใบรับรองกับ `HTTPS_ENABLED=1` ไว้ด้วยกัน — เปิดข้างเดียวล้มเหลวคนละแบบ
...
- **กลไก governance ทั้งหมดย้ายไป [`docs/GOVERNANCE.md`](docs/GOVERNANCE.md) แล้ว**
...
- **`TestConfig` ใน `tests/conftest.py` ต้อง `class TestConfig(Config)` เสมอ**
  ...
  (LANGUAGES, RATELIMIT_STORAGE_URI, THEMES, LOG_LEVEL) เทสต์จะล้มเหลวโดยหาสาเหตุไม่เจอ
...
- **ตั้งชื่องาน/หมวดในเทสต์อย่าให้ตรงกับข้อความบน UI** เช่น "ยังไม่เสร็จ"/"เสร็จแล้ว"
  เป็น label ของลิงก์ตัวกรองที่อยู่ในหน้าเสมอ `assert ... not in resp.data` จะล้มเหลวทันที
...
วิธี: ทำให้โค้ดที่เทสต์นั้นอ้างว่าคุ้มอยู่ผิดทีละจุด → เทสต์ต้องแดง → คืนโค้ดกลับ
ถ้าทำให้ผิดแล้วยังเขียว แปลว่าเทสต์นั้นไม่ได้ทดสอบอะไรเลย ให้แก้เทสต์ ไม่ใช่ปล่อยผ่าน

ต้องทำให้ผิดให้ตรงกับสิ่งที่เทสต์อ้าง — ลบ `aria-label` ออกจริง ๆ ไม่ใช่แค่แก้ตัวแปร
...
**ข้อห้ามที่เครื่องตรวจได้ มีเครื่องตรวจแล้ว** (`tests/test_declared_prohibitions.py`
— audit รอบ 14) ตรวจสองทิศ: ละเมิด = แดง · และ**แถวที่อ้างข้อความซึ่งไม่มีใน
ไฟล์นี้แล้ว = แดงเหมือนกัน** (กฎถูกถอน ด่านต้องถูกถอนตาม) — แก้ถ้อยคำของกฎที่
ถูกอ้างต้องไปอัปเดต `quote` ด้วย
...
- `DATA_ENCRYPTION_KEY` (base64 ของ 32 ไบต์ — ADR 0046) คีย์ encrypt ความลับ
  ...
  ไม่มีคีย์ plugin ปิดตัวเอง (enroll ไม่ได้ แต่แอปยังทำงานได้)
...
- **`batch_alter_table` บน SQLite ทำให้ข้อมูลเสียหายตอนเปลี่ยนชนิดคอลัมน์เป็น DATETIME**
...
- ถอน plugin ที่มีคนใช้อยู่แล้วระบบต้องไม่ล้มเหลว — ค่าที่เก็บไว้ใน `User.theme`
...
- **หลัง `flask db upgrade` ต้อง `flask plugin-install <ชนิด>/<ไอดี>` ด้วย**
  ...
  ถ้าไม่เช็ค หน้า login จะล้มเหลวทั้งหน้าด้วย `no such table`)
...
- **ไม่มีไลบรารี = ปิดตัวเอง ไม่ใช่ล้มเหลว** — ใช้กับ **plugin เต็มตัวด้วย** ไม่ใช่แค่
...
- **ทุก fixture ที่สร้างแอปต้องเดินผ่าน `_app_with_tables()`** ห้ามเรียก
  ...
  · ข้อยกเว้นเดียวคือไฟล์ที่**ตรึงฐานเป็น sqlite ของตัวเอง** ซึ่งอยู่นอกอันตราย
  นี้โดยนิยาม (`tests/test_declared_prohibitions.py` บังคับเส้นนี้)
...
### job `bare`: "ถอดแล้วระบบยังทำงานได้" ต้องวัดได้ ไม่ใช่แค่ตั้งใจ
...
- **ตัวแยกพิกัดของ zone.tab เคยล้มเหลวอย่างเงียบ ๆ มาแล้ว** — มีสองรูปแบบคือ 11 ตัว
  (`±DDMM±DDDMM`) กับ 15 ตัว (`±DDMMSS±DDDMMSS`) ถ้าเดาความยาวผิดจะได้
  ลองจิจูดที่ไม่ถูกต้องโดยไม่มี error ตอนนี้ raise ทันทีถ้ารูปแบบไม่ตรง
...
- เทสต์ทั่วไปปิด rate limit (`RATELIMIT_ENABLED = False`) ตัวจริงเทสต์ใน `tests/test_ratelimit.py`
  ...
  (ก่อน `init_app` ยังไม่มี storage จะ assert ไม่ผ่าน)
...
- ทุก request ได้ `request_id` — รับต่อจาก header `X-Request-Id` ได้ **เฉพาะที่เป็น UUID จริง**
  ค่าที่ไม่ถูกต้องถูกทิ้งแล้วสร้างใหม่ (กันคนนอกปลอม/inject log) และส่งกลับใน response header ด้วย
...
- **ข้อมูลของ plugin ให้ plugin ตอบเอง** ผ่านโมดูล `personal_data.py` ที่มี
  ...
  · plugin ตัวเดียวล้มเหลวต้องไม่ทำให้ทั้งคำขอล้ม (log แล้วใส่หมายเหตุแทน)
...
- **label ของ metric ต้องเป็น `request.endpoint` ห้ามใช้ `request.path`**
  ไม่งั้นจำนวน time series โตตามจำนวนงานในระบบ และคนนอกยิง path ที่ไม่มีอยู่จริง ให้ระเบิดได้
...
- ไล่หาจุดที่ระบบเริ่มรับไม่ไหวด้วย `NO_THRESHOLDS=1` (ต้อง "ไม่ผ่าน" เป็นเรื่องปกติ)
...
- **ทุกตารางขึ้นต้น `tdl_`** ตาราง core คือ `tdl_user` / `tdl_category` / `tdl_todo`
  ...
  ผลพลอยได้: `user` ไม่ใช่ชื่อตารางอีกแล้ว landmine reserved word หมดไปถาวร
...
เคยเกิดปัญหามาแล้วตอน Phase 2 — โค้ดที่ `inspect()` หรือ execute บน connection ตัวเดียวกับ
...
- **ระยะพวกนั้นจะเป็นจริงก็ต่อเมื่อมีอะไรรัน `purge-expired` ตามรอบ** ตัวห่อสำหรับ
  ...
  **unit จริงอยู่ที่ `deploy/systemd/` แล้ว** (P5-16) ติดตั้งด้วย
  ...
  ผ่าน `systemctl is-failed` (งานตามรอบที่เงียบตอนล้มเหลวแย่กว่าไม่มีเลย)
...
- **การต่อสายเป็นลำดับด้วยการล็อกแถวเดียวของ `tdl_audit_lock`** (ADR 0035
  ...
  **deadlock** แทน (วัดจริง: writer 8 ตัว ล้มเหลว 128 จาก 160 ครั้ง)
  ...
  ไม่มีใครแข่ง — **ทั้งสองอย่างเคยทำให้เทสต์เขียวทั้งที่โค้ดล้มเหลว**
...
### ไม่ได้ค้าง — ตัดสินแล้วว่าไม่ทำ หรือทำเสร็จแล้วแต่เก็บบริบทไว้

**หัวข้อย่อยนี้เป็นเส้นแบ่งที่เครื่องอ่านด้วย** (`scripts/whats_pending.py` หยุดนับ
ตรงนี้ — audit รอบ 14 ข้อ 4) · ของที่ปิดแล้วซึ่งนอนอยู่ใต้หัวข้อ "ยังไม่ได้ทำ"
ทำให้กองดูใหญ่กว่าความจริง แล้วตัวเลขบนหน้าเดียวที่สร้างมาเพื่อให้เชื่อได้ ก็เชื่อไม่ได้
...
- **OIDC เสร็จแล้วตั้งแต่ P5-13** (ADR 0028) core รู้จักปัจจัยหลักที่ไม่ใช่รหัสผ่าน
  ผ่าน `app/services/sso.py` (`begin`/`finish`) เท่านั้น และ **ไม่รู้จักชื่อ plugin
  ตัวไหนเลย** · ตาราง `tdl_auth_oidc_identity` เก็บแค่ `(issuer, sub) → user_id`
  ถอน plugin แล้วผู้ใช้ยังอยู่ครบ · **ที่ยังไม่มี** (ของค้างจริง อยู่ในกองข้างบน
  แล้ว): single logout · refresh token · การผูกหลาย IdP กับผู้ใช้คนเดียว
...
สรุปว่าอะไรเปลี่ยนไปหลังแต่ละเฟสอยู่ใน [`docs/ROADMAP.md`](docs/ROADMAP.md)
(ย้ายออกจากไฟล์นี้เมื่อ 2026-08-20 ตาม ADR 0065 — เพดานเต็ม) · ผลที่ค้างอยู่กับ
งานประจำวัน: CI มี **28 job (30 check)** ที่ยิงของจริงทุก push ไม่ใช่ mock

## กติกาการตรวจเอกสาร (คำสั่งเจ้าของ 2026-08-16)

**ทุกครั้งที่อัปเดตเอกสาร ตรวจ consistency ทั้ง text และสัญลักษณ์** —
เครื่องหมายสถานะในตาราง (✅/⚠️/~~ขีดฆ่า~~) · badge/รูปและเลขเวอร์ชันในนั้น ·
convention ของสัญลักษณ์ข้ามไฟล์ · **ของที่หายไปคือ bug เท่ากับของที่ผิด**:
รอบ sweep ใหญ่เคยตรวจ 26 ไฟล์แล้วประกาศสะอาด แต่พลาด ✅ ที่หายสามแถวใน
...
1. ด่านที่ "มีอยู่" กับด่านที่ "ครอบชั้นที่เกิดปัญหาจริง" เป็นคนละเรื่อง และความต่าง
   ...
   **เขียนด่านใหม่ต้องทดสอบสองทิศเสมอ**: ล้มเหลวเมื่อควรล้มเหลว และผ่านเมื่อควรผ่าน
...
3. **เครื่องมือวัดที่เราสร้างเองเพื่อพิสูจน์ว่าแก้ถูก ก็ให้ข้อมูลที่ไม่จริงได้** (Phase 7) —
   ...
   "แก้แล้ว" เป็น "ยังล้มเหลวอยู่ 87%"** — อันตรายกว่าสองข้อบน เพราะมันตอบว่าผ่าน
