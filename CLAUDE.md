...
- `pins/` — **ล็อกไฟล์ของเครื่องมือที่ CI ติดตั้งเอง** (pipenv, pip, semgrep, pa11y-ci)
  ...
  **เพิ่มไดเรกทอรีที่นี่ต้องต่อ Dependabot ให้ด้วย** ไม่งั้นแดง — pin โดยไม่มีใคร
  ...
  ซึ่งตรวจ **สองทิศ**: เจอของที่ไม่ได้อยู่ใน `pins/accepted-advisories.txt` = แดง
  และ **ID ที่ยกเว้นไว้แต่ไม่โผล่แล้ว = แดงเหมือนกัน** เพราะ `--ignore-vuln` เงียบ
  เสมอเมื่อ ID หายไป รายการยกเว้นที่ไม่มีใครถอดจะกลายเป็นตัวปิดของจริงในวันหนึ่ง
  · เหตุผลของแต่ละข้ออยู่ใน `docs/SECURITY-CADENCE.md` (`tests/test_pins_audit.py`
  บังคับให้ไฟล์กับเอกสารตรงกัน) · **ฝั่ง npm ยังไม่มีด่านที่บล็อก**
...
reverse proxy, TLS, ≥2 replica, IdP และ directory · CI มี **22 job (24 check)** ที่ยิง
