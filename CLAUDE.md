...
- **ผลวัดประสิทธิภาพจริงอยู่ใน `docs/PERFORMANCE.md`** (Phase 6) — เป้าที่ 5
  concurrent ผ่าน แต่ **p99 เฉียดเป้ามาก** และ **การรัน ≥2 replica ทำให้การเขียน
  ล้มราว 0.4% เพราะสาย audit ต่อขนานข้าม process ไม่ได้** (ข้อจำกัดที่ ADR 0015
  บันทึกไว้เองแล้วว่าต้องแก้เมื่อถึงวันที่เขียนขนานจริง — วันนั้นมาถึงแล้ว)
...
## Phase 5 ปิดแล้ว (2026-08-11) — ต่อไปคือ Phase 6 (performance)
...
reverse proxy, TLS, ≥2 replica, IdP และ directory · CI มี **18 job** ที่ยิง
ของจริงทุก push (ไม่ใช่ mock): สามยี่ห้อฐานข้อมูล, stack, SSO, LDAP, Vault,
image, timer ของงานลบข้อมูล

**บทเรียนที่ใช้ได้กับทุกเฟสถัดไป**: ด่านที่ "มีอยู่" กับด่านที่ "ครอบชั้นที่พังจริง"
เป็นคนละเรื่อง และความต่างมักอยู่ที่ **สัญญาณที่วัด** ไม่ใช่ตรรกะ
(`openssl s_client` ตอบเหมือนกันไม่ว่า server ปฏิเสธหรือไม่มีใครฟังพอร์ต ·
`flask --help` คืน 0 แม้โหลดแอปไม่สำเร็จ · fake ที่ใจดีกว่าของจริง)
