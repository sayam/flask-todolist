...
ALLOWED_LINES = {
    ...
    # readiness ต้องพิสูจน์ว่า "คุยกับฐานข้อมูลได้จริง" — SELECT 1 ไม่แตะข้อมูล
    # สักแถวและไม่ใช่ DML (ADR 0048) · ORM query ทำหน้าที่นี้ไม่ได้ตรงเท่า
    # เพราะมันพ่วง soft-delete filter และ identity map ที่ไม่เกี่ยวกับคำถามนี้
    ("health.py", 'db.session.execute(text("SELECT 1"))'),
    ...
    # อ่านอย่างเดียว (SELECT) จากตารางเวอร์ชันของ alembic ซึ่งไม่มี ORM model
    # โดยธรรมชาติ — ตารางนั้นเป็นของเครื่องมือ migration ไม่ใช่ข้อมูลของแอป
    # จึงไม่มี audit/soft delete ให้เลี่ยงตั้งแต่ต้น (หน้า lifecycle เฟส 14-06)
    ("system_info.py", "SELECT version_num FROM tdl_alembic_version"),
    # หางสายของ audit เก็บไว้บนแถวล็อกแถวเดียว (ADR 0035) — Core UPDATE โดยตั้งใจ
    # เพราะแถวนั้นไม่ใช่ข้อมูลของใคร และการให้มันไหลผ่าน ORM จะเรียก `after_flush`
