...
# พื้นของยามตัวนี้ **ตามประชากรที่หดลงเรื่อย ๆ** — สคริปต์ที่ยิงคำสั่งออกไปกำลัง
# ทยอยย้ายไป `verifiable-gates` (ADR 0077) และที่นี่เหลือ adapter ซึ่งไม่เรียก
# `subprocess.run` เลย · ลดพื้นเมื่อการย้ายทำให้เลขตก **ไม่ใช่การผ่อนเกณฑ์**
# แต่การไม่ลดคือการปล่อยให้ยามแดงค้างด้วยเหตุที่ไม่ใช่ความผิด แล้วคนจะเลิกอ่านมัน
#
...
def test_the_scan_actually_finds_the_calls_it_claims_to_check():
    ...
    assert found >= CALLS_FLOOR, (
        f"เจอ subprocess.run แค่ {found} จุด (พื้น {CALLS_FLOOR}) — "
        "ตัวสแกนน่าจะพัง หรือมีสคริปต์ย้ายออกไปโดยไม่ได้ลดพื้นตาม"
    )
