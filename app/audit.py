...
และ **การต่อสายถูก serialize ด้วยการล็อกแถวเดียวของ `tdl_audit_lock`**
(`_last_hash` — **ADR 0035**) ผู้เขียนรายที่สองจึงรอ ไม่ใช่ชนแล้วตกไป

**ห้ามอ่านบรรทัดบนแล้วเข้าใจว่าล็อกที่หางสาย** — การล็อก
`ORDER BY id DESC LIMIT 1 FOR UPDATE` บน `tdl_audit` เองคือวิธีของ ADR 0032
ซึ่ง **ถูกแทนแล้วและห้ามกลับไปใช้** (InnoDB จับ next-key lock ที่รวมช่องว่าง
ท้ายตาราง ช่องว่างเข้ากันได้กับช่องว่าง ผู้เขียนหลายรายจึงผ่านพร้อมกันแล้วไป
ชนกันตอน INSERT เป็น deadlock แทน — วัดจริง: writer 8 ตัว ตาย 128/160)
รายละเอียดอยู่ที่ `_last_hash` และ `AuditLock`
